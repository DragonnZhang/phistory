...
- The current turn includes the `ultracode` keyword and its system reminder explicitly opts this turn into Workflow.
- An UltraCode system reminder confirms that the standing session opt-in is active.
...
The `ultracode` keyword is a one-turn opt-in to Workflow orchestration. UltraCode effort mode is a standing opt-in: use Workflow for every substantive task, and work solo only for conversational or clearly trivial turns. Optimize for the most exhaustive, correct answer rather than the fastest or cheapest one. Complex tasks may run sequential workflows for understanding, design, implementation, and adversarial review. Keep the standard permission, concurrency, budget, and runtime limits.
