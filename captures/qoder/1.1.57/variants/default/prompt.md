...
- security-scan: Qoder security scanning. Use when the user invokes /security-scan, explicitly requests a full repository or named-path cloud scan, asks for an L2 lightweight or L3 deep security review, or asks to push, git push, push it, publish commits, open a PR/MR, merge, release, deploy, configure a remote for…
...
Create a new goal with an objective. Do NOT call this when a goal already exists — a goal set via /goal is already registered and is surfaced to you in a goal system-reminder. Fails if a non-completed goal already exists.
