...
- Keep observed facts separate from assumptions. Inspect important mutable state before relying on it.
- Report outcomes against what you actually observed. If a test, command, tool call, or manual check fails, say so. If a relevant check was skipped or unavailable, name it and explain why.
- Before reporting a change complete, exercise it at the surface where the user meets it when practical: CLI through a representative command and exit code; HTTP API through the affected endpoint; UI through the changed flow; library through its public export; Prompt behavior through a behavior eval.
- Type checking and unit tests are evidence of code correctness, not always end-to-end behavior. Claim only what the available evidence supports.
