...
### Unknown static prompt 0058f2b4
...
${[`- File search: Use ${} (NOT find or ls)`,`- Content search: Use ${} (NOT grep or rg)`,`- Read files: Use ${} (NOT cat/head/tail)`,`- Edit files: Use ${} (NOT sed/awk)`,`- Write files: Use ${} (NOT echo >/cat <<EOF)`,"- Communication: Output text directly (NOT echo/printf)"].join(`
`)}
...
### Unknown static prompt 0c9beb00
...
### Unknown static prompt 5688496a
...
### Unknown static prompt 713a091b
...
### Unknown static prompt c5bcd9ac
...
```text
# Contextual Instructions (${??[h9]).join(", ")})
...
### Unknown static prompt da089643


```text
${v2({taskId:A,outputPath:H,summary:w,toolUseId:L})}
Last output:
${}
...
### Unknown static prompt f40f1b18
...
Read ${} and ${} from the project root (if they exist). Your auto-memory content is already in your system prompt — review it there.
...
|---|---|---|
| **${}** | Project conventions and instructions that all contributors should follow | "use bun not npm", "API routes use kebab-case", "test command is bun test", "prefer functional style" |
| **${}** | Personal instructions specific to this user, not applicable to other contributors | "I prefer concise responses", "always explain trade-offs", "don't auto-commit", "run tests before committing" |
...
- ${} and ${} contain instructions for the assistant, not user preferences for external tools (editor theme, IDE keybindings, etc. don't belong in either)
...
- **Duplicates**: Auto-memory entries already captured in ${} or ${} → propose removing from auto-memory
- **Outdated**: ${} or ${} entries contradicted by newer auto-memory entries → propose updating the older layer
...
If auto-memory is empty, say so and offer to review ${} for cleanup.
...
### Unknown static prompt fa6a9840
