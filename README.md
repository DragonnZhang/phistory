...
> Checks for new releases daily. Archive last updated: **2026-08-25 12:07 UTC**.
...
Claude Code keeps the non-official/custom-API path as its `default` snapshot. Its additional `official` snapshot pins `claude-sonnet-5` and uses transparent forward-proxy capture so `ANTHROPIC_BASE_URL` remains unset; capture-only mode returns a dummy response locally instead of calling the model provider.
...
Last capture update: 2026-08-25 12:07 UTC
