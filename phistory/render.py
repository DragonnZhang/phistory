    ...
    lines.extend(
        [
            ...
            (
                ...
                "`claude-opus-4-8[1m]` in `official-opus-4-8`, `claude-opus-4-7[1m]` in `official-opus-4-7`, and "
                "`claude-fable-5` in `official-fable`; all five use "
                ...
                "same explicit Sonnet 5, Opus 5 1M, Opus 4.8 1M, Opus 4.7 1M, or Fable 5 model; they do not reconstruct the model that was the official default "
                "when that CLI was released. All five official archives cover the complete stable CLI history."
            ),
            "",
            (
...
def _readme_zh_markdown(rows: list[dict[str, Any]], base: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            (
                "Claude Code 的 `default` 快照保留非官方／自定义 API 路径；五个官方 API 快照分别在 `official` "
                ...
                "`claude-opus-4-7[1m]`，在 `official-fable` 中固定使用 `claude-fable-5`。五者都通过透明正向代理抓取，"
                "使 `ANTHROPIC_BASE_URL` 保持未设置。capture-only 模式会在本地返回虚拟响应，不会调用真实模型服务。"
                "这些通道的历史条目会让每个旧版 CLI 显式使用同一个 Sonnet 5、Opus 5 1M、Opus 4.8 1M、Opus 4.7 1M 或 Fable 5 模型，并不还原该版本发布时的官方默认模型。"
                "五条官方线路都覆盖完整的稳定版 CLI 历史。"
            ),
            "",
            (
